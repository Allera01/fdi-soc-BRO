...
def graficar_grafo(G, nombre, ruta_guardado=None):
    ...
    pos = layouts.get("spring", nx.spring_layout)(
        G, seed=42
    )  # Layout reproducible con semilla
...
def guardar_digrafo_edgelist_en_actual(G, nombre_archivo):
    ...
    if os.path.exists(nombre_archivo):
        print(
            f"El archivo {nombre_archivo} ya existe en el directorio actual. No se ha creado un nuevo archivo."
        )
        return
    ...
    for i in range(len(data)):
        ...
        for comentario in comentarios:
            if "snippet" in comentario and "topLevelComment" in comentario["snippet"]:
                ...
                # Añadir nodos y aristas de las respuestas
